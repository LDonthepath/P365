...
Read `docs/P365-ROADMAP-v0.1.md` in full before doing anything else in this repo. It has the current, audited status of every phase — do not assume a feature is missing or done without checking it first.
...
4. **Do not touch Phase 5-8 code** (`lib/domain/state.ts`, `risk.ts`, `intelligence.ts` and anything that would consume them) unless a human has explicitly asked for that specific phase. See the roadmap's "Explicit non-goals" — this includes not adding any trading signal, portfolio sizing, or trade execution logic under any framing.
5. **Do not add a new external dependency** (database, SaaS integration, paid API) without asking first, even if it seems like the obvious next step. Check `docs/P365-ROADMAP-v0.1.md` Phase 2 for a live example of why (a Supabase persistence layer was added, then removed, without either step being asked for).
...
7. **Reuse existing contracts, don't invent parallel ones.** `ProviderResult<T>` (`lib/data/types.ts`), `Observation`/`Event`/`Evidence`/`Context` (`lib/domain/types.ts`), and the provider file pattern in `lib/data/*.ts` (apiKey guard → fetch with `next.revalidate`+`tags:["p365-dashboard"]` → diagnostic/error handling → typed result) are the established shape. Match it.
8. **UI text is Indonesian.** Never interpolate a raw status enum (`PENDING`/`FRESH`/`PARTIAL`/`UNAVAILABLE`/etc.) directly into JSX — always go through a translated label map. This exact bug has regressed twice; see roadmap Section 9.
...
Next.js 15 (App Router), TypeScript, React Server Components + Server Actions, deployed on Vercel. No database currently in production use. Providers: FRED, Alpha Vantage, CoinGecko, ForexFactory, Federal Reserve (FOMC calendar) — all free-tier, all in `lib/data/`. Build with `npm run build` before considering any change done; it must pass with no new errors.
