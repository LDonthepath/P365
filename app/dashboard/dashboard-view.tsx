...
function ContextCard({ context, label }: { context: Context; label?: string }) { return <article className="context-card"><div className="context-card-head"><span className="context-scope">{label ?? context.scope.replaceAll("_", " ")}</span><span className="context-count">{context.observationIds.length} OBS · {context.eventIds.length} EVENT</span></div><h3>{context.statement}</h3><p>Traceable ke canonical observation/event. Tidak ada interpretasi arah pasar pada layer ini.</p></article>; }
...
export function DashboardView({ data, sessionEmail }: { data: DashboardData; sessionEmail: string }) {
  ...
  return <main className="dashboard-shell">
    ...
    <div className="dashboard-content" role="tabpanel">
      ...
      {activeMenu === "intelligence" && <div className="menu-grid"><section className="panel intelligence-panel"><div className="panel-label"><span>MARKET CONTEXT</span><span>{contexts.length} CONTEXT</span></div><h2>Context yang terverifikasi</h2><p className="lead-copy">Context hanya mengelompokkan observasi dan event yang sudah ada. Tidak ada arah pasar, regime, sentiment, liquidity, atau risk yang dihasilkan pada tahap ini.</p><div className="context-list">
        {contexts.length ? <>
          {cryptoContexts.length > 0 && <section className="context-group"><div className="panel-label"><span>CRYPTO MARKET</span><span>{cryptoContexts.length} ASSET CONTEXT</span></div><div className="context-list">{cryptoContexts.map((context) => <ContextCard context={context} label={context.id.replace("context-crypto-", "").toUpperCase()} key={context.id} />)}</div></section>}
          {macroContexts.length > 0 && <section className="context-group"><div className="panel-label"><span>MACRO</span><span>{macroContexts.length} CONTEXT</span></div><div className="context-list">{macroContexts.map((context) => <ContextCard context={context} label={MACRO_CONTEXT_LABELS[context.scope] ?? context.scope.replaceAll("MACRO_", "").replaceAll("_", " ")} key={context.id} />)}</div></section>}
          {economicEventContexts.length > 0 && <section className="context-group"><div className="panel-label"><span>ECONOMIC EVENTS</span><span>{economicEventContexts.length} CONTEXT</span></div><div className="context-list">{economicEventContexts.map((context) => <ContextCard context={context} label="Scheduled Events" key={context.id} />)}</div></section>}
        </> : <EmptyPanelNote label="context" />}
      </div></section><section className="panel"><div className="panel-label"><span>MONITOR</span><span>STATUS OPERASIONAL</span></div><h2>Apa yang perlu dipantau?</h2><div className="monitor-list"><div><strong>Observasi pasar</strong><span>{observations.length} observasi · kualitas {marketStatus}.</span></div><div><strong>Event makro</strong><span>{highImpactEvents} event berdampak tinggi saat ini.</span></div><div><strong>Kesehatan provider</strong><span>{providerStatus === "FRESH" ? "Semua provider sehat." : "Satu atau lebih provider perlu perhatian."}</span></div><div><strong>Canonical events</strong><span>{events.length} event tersedia untuk context.</span></div></div></section></div>}
