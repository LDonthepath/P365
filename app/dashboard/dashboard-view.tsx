...
function ContextCard({ context }: { context: Context }) { return <article className="context-card"><div className="context-card-head"><span className="context-scope">{context.scope.replaceAll("_", " ")}</span><span className="context-count">{context.observationIds.length} OBS · {context.eventIds.length} EVENT</span></div><h3>{context.statement}</h3><p>Traceable ke canonical observation/event. Tidak ada interpretasi arah pasar pada layer ini.</p></article>; }
