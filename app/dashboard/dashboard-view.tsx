...
function MacroObservationRow({ item, baseline, expanded, onToggle }: { item: Observation; baseline: BaselinePresentation | null; expanded: boolean; onToggle: () => void }) {
  const metadata = item.metadata ?? {};
  const unit = String(metadata.unit ?? "");
  const delta = baseline?.changeValue ?? null;
  const seriesId = String(metadata.seriesId ?? "");
  const frequency = String(metadata.frequency ?? "UNKNOWN");
  return <article className={`macro-observation-card${expanded ? " expanded" : ""}`}>
    <button type="button" className="macro-observation-summary" aria-expanded={expanded} onClick={onToggle}>
      <div className="macro-tile-header"><span className="macro-frequency">{frequency}</span><span className="macro-quality">{item.quality}</span></div>
      <div className="macro-tile-main"><div><h3>{item.subject}</h3><small>{seriesId}</small></div><strong className="macro-value">{formatMacroValue(item.value, unit)}</strong></div>
      <div className="macro-tile-footer"><span>{baseline?.baselineValue !== null && baseline?.baselineValue !== undefined ? `Ref ${formatMacroValue(baseline.baselineValue, unit)} · Δ ${formatBaselineDelta(delta)}` : `Ref ${baseline?.status ?? "MISSING"}`}</span><span>{expanded ? "TUTUP ↑" : "DETAIL ↓"}</span></div>
    </button>
    {expanded && <div className="macro-observation-detail">
      <div><strong>CURRENT VALUE</strong><span>{formatMacroValue(item.value, unit)}</span></div>
      <div><strong>REFERENCE</strong><span>{baseline?.baselineValue !== null && baseline?.baselineValue !== undefined ? formatMacroValue(baseline.baselineValue, unit) : baseline?.status ?? "MISSING"}</span></div>
      <div><strong>CHANGE</strong><span>{formatBaselineDelta(delta)}</span></div>
      <div><strong>FREQUENCY</strong><span>{frequency}</span></div>
      <div><strong>UNIT</strong><span>{unit || "—"}</span></div>
      <div><strong>OBSERVED AT</strong><span>{item.observedAt}</span></div>
      <div><strong>RELEASED AT</strong><span>{String(metadata.releasedAt ?? "Tidak disediakan provider")}</span></div>
      <div><strong>VINTAGE DATE</strong><span>{String(metadata.vintageDate ?? "—")}</span></div>
      <div><strong>SOURCE</strong><span>{String(metadata.source ?? item.sourceId)}</span></div>
      <div><strong>QUALITY</strong><span>{item.quality}</span></div>
      <div><strong>SERIES ID</strong><span>{seriesId}</span></div>
      <div><strong>RETRIEVED AT</strong><span>{item.retrievedAt}</span></div>
    </div>}
  </article>;
}
...
export function DashboardView({ data, sessionEmail }: { data: DashboardData; sessionEmail: string }) {
  ...
  const [expandedMacroSeries, setExpandedMacroSeries] = useState<string | null>(null);
