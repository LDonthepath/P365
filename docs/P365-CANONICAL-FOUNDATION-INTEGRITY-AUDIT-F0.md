...
| CAP-001C | **CLOSED / PRODUCTION-ACTIVE** | PR #69 merged as `de57880958eecdc61d60ff992f3229d8ebd13205`; production deployment is READY. Append-only Snapshot supersession is active. The first cron run after deploy at 20:51Z was `EMPTY` because the Jobless Claims event had already aged beyond the normal 90-minute reconstruction lookback, so CAP-001C could not automatically revisit those historical slots. |
| CAP-001D | **IMPLEMENTED IN PR / OWNER MERGE PENDING** | Adds a trusted POST-only targeted historical Snapshot repair path for one provider-independent Event identity. It does not widen or bypass the normal 90-minute cron lookback. Repair discovers only immutable Snapshot slots already persisted for that Event, anchors to each slot's original eventWindowId/T0/role/targetAt metadata, reconstructs canonical inputs strictly as-of the original target, and delegates correction to CAP-001C supersession. It cannot create missing logical slots or re-anchor a window from later Event revisions. |
...
| 24 Sep 2026 | CAP-001D targeted historical repair | CAP-001C reached production after the affected Jobless Claims slots had fallen outside the normal 90-minute reconstruction window. CAP-001D adds a CRON_SECRET-protected, POST-only repair surface that selects existing immutable slots by Event identity and preserves their original Event Window anchors while rebuilding only target-bounded lineage. Normal scheduler cadence/lookback remains unchanged. |
