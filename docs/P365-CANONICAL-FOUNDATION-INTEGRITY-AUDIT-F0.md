...
| Historical continuity | **PARTIAL / HIGH GAP REMAINS** | FND-003A adds an authenticated, provider-selective Observation worker with fresh acquisition and FRED-only bounded backfill. Event ingestion, external intraday scheduling, and production execution proof remain missing. |
...
6. FND-003A supports explicit provider selection, cache-bypassing forward acquisition, and FRED-only bounded backfill. Independent Event ingestion, the external intraday clock, deployment credentials, and production execution evidence remain FND-003B/C gaps.
...
| FND-003 | **HIGH / PARTIAL — FND-003A IMPLEMENTED** | Observation ingestion no longer has to depend on dashboard access: an authenticated endpoint requires explicit providers, bypasses provider cache, isolates partial failures, and permits bounded FRED-only backfill. Independent Event/FOMC ingestion, external cadence ownership, production credentials, and live write/dedupe proof remain required before FND-003 is complete. |
...
| FND-020 | **HIGH / REMEDIATED IN THIS CHECKPOINT** | Additive semantic dimensions are implemented for current canonical Observations while preserving legacy `ObservationDomain`, canonical IDs, history identity and append-only Market Memory. Current FRED/Yahoo/CoinGecko families have explicit mappings; legacy rows resolve semantics from stable machine keys without destructive backfill. |
...
| 20 Sep 2026 | FND-003A intraday ingestion control | Added an authenticated scheduler-agnostic Observation worker with mandatory provider selection, explicit FORWARD/BACKFILL modes, fresh no-cache acquisition, per-provider reporting/failure isolation, and FRED-only bounded backfill. Event ingestion, scheduler activation and production proof remain separate checkpoints. |
