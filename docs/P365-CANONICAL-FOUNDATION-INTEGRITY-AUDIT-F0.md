...
| CAP-001C | **IMPLEMENTED IN PR / OWNER MERGE PENDING** | Adds append-only Snapshot correction/supersession for defective event-window slots. CAP deterministically rebuilds the as-of slot; if the active immutable Snapshot differs and the candidate strictly improves quality/completeness without regression, a new Snapshot is appended with `supersedesSnapshotId`, `correctionCandidateSnapshotId`, and policy `append-only-snapshot-supersession-v1`. Forks/cycles/broken supersession fail closed. Repeated retries resolve the active tip and remain idempotent. |
