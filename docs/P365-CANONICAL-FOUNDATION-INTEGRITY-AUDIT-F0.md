...
**Audited ref:** EVW-001 implementation based on `main@46995ecba59fdcf68cb141bf31be434a89883f9f`
...
The current system can independently ingest and normalize selected factual Observations and Events, create evidence-backed Context, persist canonical records, query contract-compliant Observation history from durable Market Memory, and compute factual, expectation, and pricing reference baselines at their approved boundaries. SNP-001 is merged and production-deployed as an immutable point-in-time Market Snapshot capability, but no production Snapshot rows exist yet because runtime capture is intentionally not activated. EVW-001 now defines the qualified exact-event clock and PRE/T+5/T+15/T+30/T+60 capture policy on the branch. Snapshot comparison, repricing and transmission remain unimplemented and prohibited.
...
| Cross-asset baseline | **PARTIAL / SNP-001 PRODUCTION CAPABILITY + EVW-001 WINDOW POLICY IMPLEMENTED** | SNP-001 can persist/query immutable requirement-qualified reference sets. EVW-001 adds exact qualified-event timing, PRE/T+5/T+15/T+30/T+60 capture windows, snapshot-event identity binding and contamination detection. Runtime capture activation and snapshot comparison remain separate checkpoints. |
...
| Transmission reasoning | **EVW-001 WINDOW POLICY IMPLEMENTATION PASS / REPRICING + TRANSMISSION STILL MISSING** | Qualified exact HIGH point-events can now own deterministic PRE/T+5/T+15/T+30/T+60 windows with snapshot scope/event-reference validation and intervening-event contamination metadata. No price comparison, surprise, repricing, causal attribution or transmission conclusion is implemented. |
...
Event Window Policy           EVW-001 IMPLEMENTATION PASS / PREVIEW VERIFIED
