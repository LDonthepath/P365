...
**Audited ref:** `main@55027a693aec1892926368c36c67e5eda4160b83` after CAP-001D merge
...
| Transmission reasoning | **RPR-001 IMPLEMENTED ON BRANCH / OWNER REVIEW PENDING** | RPR-001 consumes CMP-001 PRE→post comparisons and applies explicit caller-owned thresholds per canonical observation key. Missing/incompatible inputs fail closed, contamination is preserved, deterministic lineage is retained, and causal attribution remains `NOT_EVALUATED`. No production threshold defaults, cross-asset transmission conclusion, State or Intelligence is activated. |
...
22B. RPR-001 Event Repricing Contract                           ← IMPLEMENTED ON BRANCH / owner review pending
22C. TRN-001 Cross-Asset Transmission                          ← after RPR-001
23. Demand-driven Macro + Crypto + Gold evidence completion    ← only when downstream contract requires it
