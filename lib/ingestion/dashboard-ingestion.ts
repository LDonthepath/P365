...
export async function ingestDashboardData(): Promise<DashboardIngestion> { const [macroResult, alphaVantageCryptoResult, coinDeskResult, calendarResult, cryptoMarketResult, fredResult, fomcResult] = await Promise.allSettled([fetchMacroNews(6), fetchAlphaVantageCryptoNews(4), fetchCoinDeskNews(6), fetchEconomicCalendar(6), fetchCryptoMarketObservations(["BTC", "ETH"]), fetchFredMacroObservations(), fetchFomcEvents()]); return { macroNews: resultOrEmpty(macroResult, "alpha-vantage"), alphaVantageCryptoNews: resultOrEmpty(alphaVantageCryptoResult, "alpha-vantage"), coinDeskNews: resultOrEmpty(coinDeskResult, "coindesk-rss"), calendar: resultOrEmpty(calendarResult, "forex-factory"), cryptoMarket: resultOrEmpty(cryptoMarketResult, "coingecko"), fred: resultOrEmpty(fredResult, "fred"), fomc: resultOrEmpty(fomcResult, "federal-reserve") }; }
