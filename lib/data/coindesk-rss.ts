...
export async function fetchCoinDeskNews(limit = 6): Promise<ProviderResult<NewsItem>> {
  try {
    ...
    if (!res.ok) return providerResult("coindesk-rss", "ERROR", [], `CoinDesk HTTP ${res.status}`);
    ...
    const data = items.slice(0, limit).flatMap((item, index): NewsItem[] => {
      ...
      return [{ id: item.link ?? `coindesk-${index}`, category: "CRYPTO", source: "CoinDesk", publishedAt: publishedAt.toISOString(), title: stripHtml(item.title), summary: stripHtml(item.description ?? ""), url: item.link }];
    });
    return providerResult("coindesk-rss", data.length > 0 ? "SUCCESS" : "EMPTY", data);
  } catch (error) {
    return providerResult("coindesk-rss", "ERROR", [], error instanceof Error ? error.message : "CoinDesk request failed");
  }
}
