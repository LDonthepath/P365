...
const KEY = process.env.P365_MEMORY_WRITE_KEY;

function config() {
  if (!URL || !KEY) throw new Error("Supabase Market Memory requires SUPABASE_URL and P365_MEMORY_WRITE_KEY");
...
function mapRow(row: Record<string, unknown>): MarketMemoryRecord { return { id: String(row.id), type: row.record_type as MarketMemoryRecord["type"], recordedAt: String(row.created_at), effectiveAt: String(row.effective_at), sourceId: String(row.source_id), canonicalId: String(row.canonical_id), payloadHash: typeof row.payload_hash === "string" ? row.payload_hash : undefined }; }
...
export class SupabaseMarketMemoryStore implements MarketMemoryStore {
  ...
  async append(input: MarketMemoryAppendInput) { return (await this.appendMany([input]))[0]; }
  async appendMany(inputs: MarketMemoryAppendInput[]) {
    if (!inputs.length) return [];
    const rows = inputs.map((input) => { const value = canonical(input); const payload = value.item; return { record_type: value.type, canonical_id: value.item.id, source_id: value.sourceId, captured_at: new Date().toISOString(), effective_at: value.effectiveAt, payload, payload_hash: hash(payload), observation_ids: value.type === "OBSERVATION" ? [value.item.id] : [], event_ids: value.type === "EVENT" ? [value.item.id] : [], evidence_ids: value.type === "EVIDENCE" ? [value.item.id] : [] }; });
    const response = await this.request("market_memory?on_conflict=dedupe_key", { method: "POST", headers: { Prefer: "resolution=ignore-duplicates,return=representation" }, body: JSON.stringify(rows) });
    ...
    return rows.map((row) => { const match = byKey.get(`${row.record_type}:${row.canonical_id}:${row.effective_at}`); if (!match) throw new Error(`Market Memory row missing after persistence: ${row.canonical_id}`); return mapRow(match); });
  }
