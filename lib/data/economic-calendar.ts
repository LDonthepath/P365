...
function isForexFactoryEvent(value: unknown): value is ForexFactoryEvent {
  ...
  return typeof item.title === "string" && item.title.trim().length > 0 && typeof item.country === "string" && item.country.trim().length > 0 && typeof item.date === "string" && item.date.trim().length > 0;
}

export async function fetchEconomicCalendar(limit = 6): Promise<ProviderResult<CalendarEvent>> {
  const now = new Date();
  try {
    const res = await fetch(FOREX_FACTORY_URL, { next: { revalidate: 3600, tags: ["p365-dashboard"] }, signal: AbortSignal.timeout(10_000), headers: { Accept: "application/json" } });
    ...
    const events = validItems.flatMap((item, index): CalendarEvent[] => {
    ...
    }).sort((a, b) => new Date(a.dateISO).getTime() - new Date(b.dateISO).getTime()).slice(0, limit);

    return providerResult("forex-factory", events.length > 0 ? "SUCCESS" : "EMPTY", events, events.length > 0 ? undefined : "ForexFactory returned no upcoming events in the current weekly feed");
  } catch (error) {
